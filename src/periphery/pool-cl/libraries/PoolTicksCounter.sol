...
library PoolTicksCounter {
        ...
        returns (uint32 initializedTicksLoaded)
    {
...

        {
            ...
            int16 wordPos = int16((tickBefore / tickSpacing) >> 8);
            uint8 bitPos = uint8(uint24((tickBefore / tickSpacing) % 256));

            int16 wordPosAfter = int16((tickAfter / tickSpacing) >> 8);
            uint8 bitPosAfter = uint8(uint24((tickAfter / tickSpacing) % 256));
